...
MAIN_REQUESTS = pc.Counter('main_page_requests_total',
        'total requests for the / route.')
...
def main():
    MAIN_REQUESTS.inc()
    next_run = (db.session.query(ApschedulerJobs)
    .filter_by(id='run_loop').first())
    if next_run:
        next_run = datetime.utcfromtimestamp(next_run.next_run_time)
    if request.method == 'POST':
        current_app.apscheduler.add_job(id='run_now', replace_existing=True,
                                            func=process)
    ...
    maintenances = (MaintCircuit.query
    ...
    next_url = url_for('main.main', page=maintenances.next_num) \
        if maintenances.has_next else None
    prev_url = url_for('main.main', page=maintenances.prev_num) \
        if maintenances.has_prev else None
    return render_template('main.html', title='main',
                           upcoming=maintenances.items, recent=recent,
...
def maintenances():
    page = request.args.get('page', 1, type=int)
    maintenances = Maintenance.query.paginate(page,
                current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('main.maintenances', page=maintenances.next_num) \
        if maintenances.has_next else None
    prev_url = url_for('main.maintenances', page=maintenances.prev_num) \
        if maintenances.has_prev else None
    return render_template('maintenances.html', title='main',
...
def providers():
    page = request.args.get('page', 1, type=int)
    providers = Provider.query.paginate(page,
                current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('main.providers', page=providers.next_num) \
        if providers.has_next else None
    prev_url = url_for('main.providers', page=providers.prev_num) \
        if providers.has_prev else None
    return render_template('providers.html', title='providers',
                           providers=providers.items, next_url=next_url,
                           prev_url=prev_url)
...
def circuits():
    ...
    if form.validate_on_submit():
        ...
        circuit = Circuit(provider_cid=form.provider_cid.data,
                          a_side=form.a_side.data, z_side=form.z_side.data,
    ...
    return render_template('circuits.html', title='circuits', form=form,
                           circuits=circuits.items, next_url=next_url,
                           prev_url=prev_url)
...
def maintenance_detail(maintenance_id):
    ...
    page = request.args.get('page', 1, type=int)
    circuits = MaintCircuit.query.filter_by(
        maint_id=maintenance_id).all()
    return render_template('maintenance_detail.html', title='Maintenance Info',
                           circuits=circuits, maintenance=maintenance,
                           updates=updates)
...
def circuit_detail(circuit_id):
    ...
    page = request.args.get('page', 1, type=int)
    maints = MaintCircuit.query.filter_by(
        circuit_id=circuit_id).order_by(
        desc(MaintCircuit.date)).paginate(page,
            current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('main.circuit_detail', page=maints.next_num,
                       circuit_id=circuit_id) \
    if maints.has_next else None
    prev_url = url_for('main.circuit_detail', page=maints.prev_num,
                       circuit_id=circuit_id) \
    if maints.has_prev else None

    return render_template('circuit_detail.html', circuit=circuit,
                           maints=maints.items, next_url=next_url,
...
def provider_detail(provider_id):
    ...
    circuits = Circuit.query.filter_by(provider_id=provider_id) \
               .paginate(page, current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('main.provider_detail', page=circuits.next_num,
                       provider_id=provider_id) \
    if circuits.has_next else None
    prev_url = url_for('main.provider_detail', page=circuits.prev_num,
                       provider_id=provider_id) \
    if circuits.has_prev else None
    return render_template('provider_detail.html', circuits=circuits,
                           provider=provider, next_url=next_url,
                           prev_url=prev_url, all_circuits=all_circuits)
